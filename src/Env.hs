...
module Env(Env,StaticEnv,DynEnv,DynId,Subst,mappedTo,singleEnv,listEnv,lookupEnv,closeEnv,openEnv,mapEnv
          ,valuesEnv,assocsEnv,keysEnv) where
...
instance Ord k => Semigroup (Env k a) where
  (Env p) <> (Env q) = Env (Map.union p q)
