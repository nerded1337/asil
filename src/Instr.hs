...
{-# LANGUAGE EmptyDataDecls, GADTs, KindSignatures, ScopedTypeVariables,
    MultiParamTypeClasses, FunctionalDependencies, TypeFamilies, AllowAmbiguousTypes,
...
instance IsSuper a a                                  -- reflexivity
instance (IsSuper a b, IsSuper b c) => IsSuper a c    -- transitivity
...
class IsNumeric a
instance IsNumInt a => IsNumInt a
...
class IsExpr' d a b | a -> b, a -> d where
  toExpr' :: d -> a -> Expr b
...
runInstr :: Instr a -> L.Instr
runInstr (Instr f) = fst $ f 1 (\n _ l -> (l, n))
...
valToVal v = case v of
  ...
  ValProp v (Key s) -> L.Val_Prop (valToVal v) s
...
ifte g t e = fmap toExpr (ifte' g t e)
